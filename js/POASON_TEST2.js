...
loader.load(
  ...
  (font) => {
    ...
    const matOxideText = new THREE.MeshMatcapMaterial({ color: 0xff0d0d });
    ...
    meshMetalText = new THREE.Mesh(geoMetalText, matMetalText);
    meshOxideText = new THREE.Mesh(geoOxideText, matOxideText);
    meshSiliconText = new THREE.Mesh(geoSiliconText, matSiliconText);
    meshGateText = new THREE.Mesh(geoGateText, matGSDText);
    meshSourceText = new THREE.Mesh(geoSourceText, matGSDText);
    meshDrainText = new THREE.Mesh(geoDrainText, matGSDText);

    meshMetalText.position.set(-200, 200, 0);
    meshOxideText.position.set(-85, 200, 0);
    meshSiliconText.position.set(100, 200, 0);
    meshGateText.position.set(-200, 0, 50);
...
function addPoints() {
  ...
  for (let i = 0; i < ver02.length; i = i + 3) {
    const x =
      i >= 107 * 3 * 56
        ? parseFloat(ver00[i]) * 4 + 250
        : parseFloat(ver00[i]) * 20;
    ...
    const z = parseFloat(
      ver00[i + 2] * 24 * 5 -
        ((ver00[i + 2] - ver02[i + 2]) * 5 * Dslider * 10 +
          (ver00[i + 2] * 5 - ver20[i + 2]) * Gslider * 20)
    );
...
function render() {
  meshMetalText.quaternion.copy(camera.quaternion);
  meshOxideText.quaternion.copy(camera.quaternion);
  meshSiliconText.quaternion.copy(camera.quaternion);
  meshGateText.quaternion.copy(camera.quaternion);
  meshSourceText.quaternion.copy(camera.quaternion);
  meshDrainText.quaternion.copy(camera.quaternion);
